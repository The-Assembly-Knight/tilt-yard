#include <stdio.h>

#include "include/tiltyard_API.h"

#define YARD_SIZE 8

int *test_func(Yard **main_yard)
{
	Yard *my_yard =  tiltyard_create(sizeof(int) * YARD_SIZE);
	int *int_array = tiltyard_calloc(my_yard, sizeof(int) * YARD_SIZE);
	
	*main_yard = my_yard;

	if (!int_array) return NULL;

	size_t i = 0;
	for (i = 0; i < YARD_SIZE; i++) {
		int_array[i] = (int)i + 10;
	}

	return int_array;
}

int main(void)
{
	Yard *main_yard = NULL;
	int *my_array = test_func(&main_yard);
	
	if (!my_array) return 1;

	if (!main_yard) return 1;

	size_t i = 0;
	for (i = 0; i < YARD_SIZE; i++) {
		printf("The content of the current pointer is: %i\n", my_array[i]);
	}

	printf("This is the current marker value: %zu\n", tiltyard_marker(main_yard));

	tiltyard_obliterate(&main_yard);

	if (main_yard)
		printf("It is still alive :(\n");

	size_t mark = tiltyard_marker(main_yard);

	if (mark == 0) return 1;

	printf("This is the value of the current mark: %zu\n", mark);

	return 0;
}
